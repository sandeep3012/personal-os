...
import 'package:feature_finance/src/domain/events/transaction_created_event.dart';
...
void main() {
  group('TransactionCreatedEvent', () {
    ...
    test('holds all fields including optional ones', () {
      ...
      expect(event.amount, Decimal.parse('500'));
      expect(event.currency, 'INR');
      expect(event.categoryId, const CategoryId('cat-food'));
      expect(event.payeeName, 'Grocery Store');
    ...
    test('categoryId and payeeName are null when omitted', () {
      final event = TransactionCreatedEvent(
        transactionId: const TransactionId('txn-2'),
        workspaceId: 'ws-1',
        accountId: const AccountId('acc-1'),
