...
/// [IEventDatabaseExecutor], built specifically to understand the exact SQL
/// shapes [EventDao] emits — mirrors Notes' `InMemoryNoteDatabaseExecutor`
/// exactly, scoped to Calendar's single table.
///
/// This is the Calendar feature's default persistence until a concrete
...
/// Calendar feature is fully usable — data simply does not survive an app
/// restart on its own (see `FileBackedEventDatabaseExecutor` for that).
final class InMemoryEventDatabaseExecutor implements IEventDatabaseExecutor {
  final Map<String, List<Map<String, Object?>>> _tables = {
    EventsSchema.eventsTable: <Map<String, Object?>>[],
  };
  ...
  // ── Snapshot / restore (backs InMemoryEventTransactionRunner) ────────────
  ...
  Future<int> execute(String sql, [List<Object?> arguments = const []]) async {
    ...
    throw UnsupportedError('InMemoryEventDatabaseExecutor: unrecognized '
  ...
  int _insert(String sql, List<Object?> args) {
    ...
    final table = _tables[EventsSchema.eventsTable]!;
