...
/// [INoteDatabaseExecutor], built specifically to understand the exact SQL
/// shapes [NoteDao] emits — mirrors Goals' `InMemoryGoalDatabaseExecutor`
/// exactly, scoped to Notes' single table.
///
/// This is the Notes feature's default persistence until a concrete storage
/// engine (Drift, Isar, SQLite, etc.) is approved via ADR — same status as
/// Finance/Goals' equivalent. It genuinely stores rows and evaluates
/// `WHERE`/`ORDER BY`/`LIMIT OFFSET` against them, so the Notes feature is
/// fully usable — data simply does not survive an app restart on its own
/// (see `FileBackedNoteDatabaseExecutor` for that).
final class InMemoryNoteDatabaseExecutor implements INoteDatabaseExecutor {
  final Map<String, List<Map<String, Object?>>> _tables = {
    NotesSchema.notesTable: <Map<String, Object?>>[],
  };
  ...
  // ── Snapshot / restore (backs InMemoryNoteTransactionRunner) ─────────────
  ...
  Future<int> execute(String sql, [List<Object?> arguments = const []]) async {
    ...
    throw UnsupportedError('InMemoryNoteDatabaseExecutor: unrecognized '
  ...
  int _insert(String sql, List<Object?> args) {
    ...
    final table = _tables[NotesSchema.notesTable]!;
