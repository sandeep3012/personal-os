...
/// Persists Habits data to a JSON file on disk, so it survives a full app
/// restart — unlike a bare [InMemoryHabitDatabaseExecutor]. Mirrors Finance's
/// `FileBackedFinanceDatabaseExecutor` exactly.
final class FileBackedHabitDatabaseExecutor implements IHabitDatabaseExecutor {
  FileBackedHabitDatabaseExecutor._(this._engine, this._file);

  final InMemoryHabitDatabaseExecutor _engine;
  final File _file;

  /// The in-memory engine backing this executor. Exposed so
  /// [FileBackedHabitTransactionRunner] can run a multi-statement unit of
  /// work directly against it and persist once at the end.
  InMemoryHabitDatabaseExecutor get engine => _engine;

  /// Loads [file]'s existing contents (if any) into a fresh
  /// [InMemoryHabitDatabaseExecutor] and returns an executor backed by it.
  ///
  /// If [file] does not exist yet (first launch), starts from an empty
  /// store — identical to a fresh [InMemoryHabitDatabaseExecutor].
  static Future<FileBackedHabitDatabaseExecutor> open(File file) async {
    final engine = InMemoryHabitDatabaseExecutor();
    ...
    return FileBackedHabitDatabaseExecutor._(engine, file);
  }
  ...
  ///
  /// Public so [FileBackedHabitTransactionRunner] can call it exactly once
