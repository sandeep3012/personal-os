...
/// [IHabitDatabaseExecutor], built specifically to understand the exact SQL
/// shapes [HabitDao] emits — mirrors Finance's
/// `InMemoryFinanceDatabaseExecutor` exactly, scoped to Habits' single table
/// (no transfer-pair-style JOIN handling is needed here).
///
/// This is the Habits feature's default persistence until a concrete storage
...
/// `WHERE`/`ORDER BY`/`LIMIT OFFSET` against them, so the Habits feature is
/// fully usable — data simply does not survive an app restart on its own
/// (see `FileBackedHabitDatabaseExecutor` for that).
final class InMemoryHabitDatabaseExecutor implements IHabitDatabaseExecutor {
  final Map<String, List<Map<String, Object?>>> _tables = {
    HabitsSchema.habitsTable: <Map<String, Object?>>[],
  };
  ...
  // ── Snapshot / restore (backs InMemoryHabitTransactionRunner) ─────────────
  ...
  Future<int> execute(String sql, [List<Object?> arguments = const []]) async {
    ...
    throw UnsupportedError('InMemoryHabitDatabaseExecutor: unrecognized '
  ...
  int _insert(String sql, List<Object?> args) {
    ...
    final table = _tables[HabitsSchema.habitsTable]!;
