...
/// [IGoalDatabaseExecutor], built specifically to understand the exact SQL
/// shapes [GoalDao] emits — mirrors Finance's
/// `InMemoryFinanceDatabaseExecutor` exactly, scoped to Goals' single table
/// (no transfer-pair-style JOIN handling is needed here).
///
/// This is the Goals feature's default persistence until a concrete storage
...
/// `WHERE`/`ORDER BY`/`LIMIT OFFSET` against them, so the Goals feature is
/// fully usable — data simply does not survive an app restart on its own
/// (see `FileBackedGoalDatabaseExecutor` for that).
final class InMemoryGoalDatabaseExecutor implements IGoalDatabaseExecutor {
  final Map<String, List<Map<String, Object?>>> _tables = {
    GoalsSchema.goalsTable: <Map<String, Object?>>[],
  };
  ...
  // ── Snapshot / restore (backs InMemoryGoalTransactionRunner) ─────────────
  ...
  Future<int> execute(String sql, [List<Object?> arguments = const []]) async {
    ...
    throw UnsupportedError('InMemoryGoalDatabaseExecutor: unrecognized '
  ...
  int _insert(String sql, List<Object?> args) {
    ...
    final table = _tables[GoalsSchema.goalsTable]!;
