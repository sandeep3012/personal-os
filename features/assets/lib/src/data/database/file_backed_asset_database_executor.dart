...
/// Persists Assets data to a JSON file on disk, so it survives a full app
/// restart — unlike a bare [InMemoryAssetDatabaseExecutor]. Mirrors Notes'
/// `FileBackedNoteDatabaseExecutor` exactly.
final class FileBackedAssetDatabaseExecutor implements IAssetDatabaseExecutor {
  FileBackedAssetDatabaseExecutor._(this._engine, this._file);

  final InMemoryAssetDatabaseExecutor _engine;
  final File _file;

  /// The in-memory engine backing this executor. Exposed so
  /// [FileBackedAssetTransactionRunner] can run a multi-statement unit of
  /// work directly against it and persist once at the end.
  InMemoryAssetDatabaseExecutor get engine => _engine;

  /// Loads [file]'s existing contents (if any) into a fresh
  /// [InMemoryAssetDatabaseExecutor] and returns an executor backed by it.
  ///
  /// If [file] does not exist yet (first launch), starts from an empty
  /// store — identical to a fresh [InMemoryAssetDatabaseExecutor].
  static Future<FileBackedAssetDatabaseExecutor> open(File file) async {
    final engine = InMemoryAssetDatabaseExecutor();
    ...
    return FileBackedAssetDatabaseExecutor._(engine, file);
  }
  ...
  ///
  /// Public so [FileBackedAssetTransactionRunner] can call it exactly once
