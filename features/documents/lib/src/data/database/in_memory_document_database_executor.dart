...
/// [IDocumentDatabaseExecutor], built specifically to understand the exact SQL
/// shapes [DocumentDao] emits — mirrors Notes' `InMemoryNoteDatabaseExecutor`
/// exactly, scoped to Documents's single table.
///
/// This is the Documents feature's default persistence until a concrete
/// storage engine (Drift, Isar, SQLite, etc.) is approved via ADR — same
/// status as Notes/Finance/Goals' equivalent. It genuinely stores rows and
/// evaluates `WHERE`/`ORDER BY`/`LIMIT OFFSET` against them, so the
/// Documents feature is fully usable — data simply does not survive an app
/// restart on its own (see `FileBackedDocumentDatabaseExecutor` for that).
final class InMemoryDocumentDatabaseExecutor implements IDocumentDatabaseExecutor {
  final Map<String, List<Map<String, Object?>>> _tables = {
    DocumentsSchema.documentsTable: <Map<String, Object?>>[],
  };
  ...
  // ── Snapshot / restore (backs InMemoryDocumentTransactionRunner) ────────────
  ...
  Future<int> execute(String sql, [List<Object?> arguments = const []]) async {
    ...
    throw UnsupportedError('InMemoryDocumentDatabaseExecutor: unrecognized '
  ...
  int _insert(String sql, List<Object?> args) {
    ...
    final table = _tables[DocumentsSchema.documentsTable]!;
